...
type ActionWithPayload<T extends Type> = {
  ...
  payload: T extends
    ...
    | 'remove-tag'
    ? string
    : T extends 'add-member' | 'remove-nmember'
      ? MemberPayload
      : T extends 'reset'
        ? undefined
        : never
...
const taskReducer = (state: TaskState, action: ActionWithPayload<Type>) => {
  const { task } = state
  ...
  switch (action.type) {
    ...
    case 'add-tag':
      ...
      break
    case 'add-member':
      console.log(task.members, action.payload)
    ...
    case 'remove-member':
      task.members = task.members.filter(
        (member) => member.id !== (action.payload as MemberPayload).id
      )
      break
    case 'reset':
      task.members = []
